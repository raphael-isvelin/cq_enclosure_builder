...
| `export_printables` -> `None` | *none* | Export one STL per printable. By default, one for the `lid` and for the `box`. Some parts can require additional prints; any element added to `Part#additional_printables` will also be exported.</li></ul>  |
...
| (value) `debug`: `cq.Assembly` | *N/A* | Debug elements: footprints, holes, panels masks, printables, and other debug elements added by the parts (`Part#debug_objects`).
...
|-------------|------------|-------------|
| `build` -> [Part](./src/cq_enclosure_builder/part.py)  | <ul><li>`category_name: str`</li><li>`part_type: str`</li><li>`**kwargs: Any`: any parameter needed by the part you're building.</li></ul> | |
...
| `set_default_types` -> `None`  | <ul><li>`defaults: Dict[str, str]`: the default part type to use per category; for example: `{"<category_name>": '<a type from that category>}`</li></ul> | Set the default part for any category, so you don't have to repeat `part_type="<type>"` each time you're building a part. |
...
| `assemble`  | *none* | Should be called before using the values below. |
