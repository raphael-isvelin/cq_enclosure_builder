...
@register_part("potentiometer", "XXX")
class PotentiometerXxxPart(GenericThreadedWithStopPart):
    """
    Potentiometer connector XXX
    ...
    def __init__(self, enclosure_wall_thickness):
        ...
        super().__init__(
            ...
            nut_thickness=2.25,
            margin_after_nut=0.89,

            pyramid_taper=70,

            dents_specs=[
                ( (1.15, 2.5, 2.2), (-(11.85-(thread_diameter/2)-(2.2/2)), 0)  )
            ],
            dent_size_error_margin=0.6,
            dent_thickness_error_margin=0.2
        )

        thread_and_button_depth = 19.4

        footprint_out_button_cap_diameter = 14.8
        footprint_out_button_cap_thickness = 16.35
        ...
        footprint_in = (
            cq.Workplane("front")
                ...
                .translate([0, 0, -inside_footprint_depth])
                .add(
                    cq.Workplane("front")
                        .box(15.1, 21.2-(17.2/2), 3.4, centered=(True, True, False))
                        .translate([0, 17.2/2, -3.4])
                )
        )
        footprint_out = (
            cq.Workplane("front")
