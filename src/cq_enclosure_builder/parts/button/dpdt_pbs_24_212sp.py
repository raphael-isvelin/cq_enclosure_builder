...
class ButtonDpdtPbs24212SpPart(GenericThreadedWithStopPart):
    ...
    def __init__(self, enclosure_wall_thickness, button_cap = CAP_23_75_x_11_9):
        ...
        inside_footprint_thickness = 16.2
        footprint_in = (
            cq.Workplane("front")
                .box(*self.inside_footprint, inside_footprint_thickness, centered=(True, True, False))
                .translate([0, 0, -inside_footprint_thickness])
        )
        
        self.outside_footprint = (self.thread_diameter, self.thread_diameter)
        footprint_out_thickness_without_knob = 20.8 - self.block_thickness - self.enclosure_wall_thickness
        footprint_out = (
            cq.Workplane("front")
