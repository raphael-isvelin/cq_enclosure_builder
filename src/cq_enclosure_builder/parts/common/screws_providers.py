...
from cq_enclosure_builder.parts.common.generic_screw_provider import GenericScrewProvider
from cq_enclosure_builder.parts.common.hole_type import HoleType


# TODO refactor these classes a bit:
#  e.g. the 'LargeBlock' classes are similar to the default ones with 10 added to everything
...
class LargeBlockFlatHeadScrewProvider:
    SCREW_CLASS = FlatHeadScrew
    SCREW_MODEL_NAME: str = "aliexpress"
    SCREW_SIZE_REFERENCES: Dict[str, str] = {
        "m1.4": "M1.4-0.3",
    ...
    COUNTER_SUNK_BLOCK_SIZES: Dict[str, Tuple[int, int]] = {
        "m1.4": (3+3, 3+3),
        "m2": (4.6+3, 4.6+3),
        "m2.5": (5.6+3, 5.6+3),  # TODO double-check once delivered
        "m3": (6.8+3, 6.8+3),
        "m3.5": (7.8+3, 7.8+3),  # TODO double-check once delivered
        "m4": (8.6+3, 8.6+3),
        "m5": (10.4+3, 10.4+3),
    }
    HOLE_TYPE: HoleType = HoleType.THREADED_HOLE
...
class LargeBlockHeatSetScrewProvider:
    SCREW_CLASS = HeatSetNut
    SCREW_MODEL_NAME: str = "Hilitchi"
    SCREW_SIZE_REFERENCES: Dict[str, str] = {
        "m2": "M2-0.4-8",
