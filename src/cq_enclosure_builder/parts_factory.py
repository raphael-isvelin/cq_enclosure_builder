...
class PartFactoryMeta(type):
    ...
    def register_part(cls, category: str, part_type: str, part_class: Type[Part]) -> None:
        ...
        if category not in cls.part_registry:
            ...
            list_method_name = f"list_available_types_of_{category.lower()}"
    ...
    def _list_types_method(category: str):
        """Generate a dynamic `list_available_types_of_` method for a given category."""
        def method(cls) -> list:
    ...
    def _list_parameters_method(category: str):
        ...
        def method(cls, part_type=None) -> List[dict[str, Any]]:
            ...
            if not part_class:
                available_part_types_method = getattr(cls, f"list_available_types_of_{category}", None)
...
class PartFactory(PartsFactoryProtocol, metaclass=PartFactoryMeta):
    ...
    Similarly, a method named `list_available_types_of_<category>` is generated to list 
    the available types for a given category. For the "jack" category, 
    `list_available_types_of_jack` will list all registered jack types.

    Usage:
    ------
    - List existing categories: `PartFactory.list_categories()`
    - List available types: `PartFactory.list_available_types_of_<category>()`
    ...
    def build(cls, category: str, **kwargs: Any) -> Part:
        ...
        if not part_type:
            ...
            if not part_type:
                raise ValueError(f"No part_type provided to build method, and no default provided for category '{category}'. See PartFactory#list_available_types_of_{category}. See PartFactory#set_default_types.")
